...
import './DocumentViewer.css'; // Reuse the same CSS from supplies

function EquipmentDocumentViewer({ equipment, isOpen, onClose }) {
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true);
      const docs = await EquipmentAPI.getEquipmentDocuments(equipment._id);
      setDocuments(docs);
    } catch (error) {
  ...
  const handleDownload = async (doc) => {
    try {
      await EquipmentAPI.downloadEquipmentDocument(equipment._id, doc.index, doc.filename);
    } catch (error) {
  ...
  const handleDelete = async (doc) => {
    ...
    try {
      setLoading(true);
      await EquipmentAPI.deleteEquipmentDocument(equipment._id, doc.index);
      await loadDocuments();
    } catch (error) {
...
const handleUpload = async () => {
  ...
  try {
    setUploading(true);
    await EquipmentAPI.uploadEquipmentDocument(equipment._id, selectedFile);
  ...
  return (
    <div className="document-viewer-overlay" onClick={onClose}>
      <div className="document-viewer-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h3>Documents - {equipment.name}</h3>
<p className="doc-item-code">Item Code: {equipment.itemCode}</p>
...
<div className="doc-upload-section">
  ...
  <div 
    ...
    onDrop={handleDrop}
    onClick={() => document.getElementById('equipDocFileInput').click()}
  >
    <input 
      type="file" 
      id="equipDocFileInput"
...
<div className="doc-list-section">
          ...
          {loading ? (
            ...
            <div className="doc-empty">
              ...
              <small>Documents are uploaded when adding new equipment</small>
            </div>
          ) : (
          ...
          <div className="doc-preview-overlay" onClick={closePreview}>
            <div className="doc-preview-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="preview-content">
                <img 
                  src={EquipmentAPI.getEquipmentDocumentUrl(equipment._id, previewDoc.index)}
